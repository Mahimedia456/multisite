...
export default function KnowledgeForm({ brandSlug, lang = "de" }) {
  const { slug } = useParams();
  const { form, loading, error } = useBrandKnowledgeForm(brandSlug, slug);
  ...
  async function submitForm(e) {
    ...
    try {
      const result = await submitBrandKnowledgeForm({
        brandSlug,
        formSlug: slug,
        payload: {
          data: values,
  ...
  return (
    <main className="bg-[#f6f8fb]">
      <section className="mx-auto max-w-4xl px-5 py-16">
        <Link
          to="/knowledge"
          className="inline-flex items-center rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-black text-[#007ab3] shadow-sm"
        >
        ...
        <div className="mt-8 rounded-[36px] border border-slate-200 bg-white p-8 shadow-sm md:p-10">
          ...
          {successMessage ? (
            <div className="mt-6 rounded-3xl border border-green-200 bg-green-50 p-5 text-sm font-bold leading-6 text-green-700">
              {successMessage}
            </div>
          ) : null}

          {submitError ? (
            <div className="mt-6 rounded-3xl border border-red-200 bg-red-50 p-5 text-sm font-bold leading-6 text-red-700">
              {submitError}
            </div>
          ) : null}
          ...
          <form onSubmit={submitForm} className="mt-8 space-y-5">
            ...
            <button
              type="submit"
              disabled={sending}
              className="inline-flex w-full items-center justify-center rounded-2xl bg-[#007ab3] px-6 py-4 text-sm font-black text-white shadow-lg shadow-[#007ab3]/20 transition hover:bg-[#005f8c] disabled:cursor-not-allowed disabled:opacity-60"
            >
              {sending
                ? lang === "en"
                  ? "Sending..."
