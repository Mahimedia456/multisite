...
export function useBrandBlogs(slug, options = {}) {
  const {
    limit = 100,
    search = "",
    category = "",
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const url = `${API_BASE}/public/${encodeURIComponent(slug)}/blogs?${params.toString()}`;
        ...
        console.error("[useBrandBlogs] error:", e);
...
export function useBrandBlogDetail(slug, blogSlug) {
  const [loading, setLoading] = useState(true);
  const [err, setErr] = useState("");
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        console.log("[useBrandBlogDetail]", {
          slug,
          blogSlug,
          url,
          status: res.status,
        ...
        console.error("[useBrandBlogDetail] error:", e);
