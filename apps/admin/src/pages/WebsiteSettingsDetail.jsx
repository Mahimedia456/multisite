...
function PageTable({ title, icon, rows, onToggle, savingId }) {
  return (
    <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900">
      <div className="flex items-center gap-3 border-b border-slate-100 bg-slate-50 px-6 py-5 dark:border-white/10 dark:bg-slate-950/60">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">{rows.length} pages</p>
        </div>
      </div>
...
export default function WebsiteSettingsDetail() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!res.ok || !json?.ok) {
        throw new Error(json?.message || "Failed to load website settings");
      }
      ...
      alert(e.message || "Failed to load website settings");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function toggle(page, value) {
    ...
    setData((prev) => {
      const updateList = (list) =>
        list.map((p) => (p.id === page.id && p.type === page.type ? { ...p, is_visible: value } : p));
    ...
    try {
      const res = await apiFetch(`/admin/website-settings/${brandId}/${page.type}/${page.id}`, {
        method: "PUT",
        ...
        <div className="mt-4 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <div className="text-xs font-black uppercase tracking-[0.2em] text-[#007ab3]">
              Website Visibility Detail
            </div>
            <h1 className="mt-1 text-2xl font-black text-gray-950 dark:text-white">
              {brand?.name || "Brand Website Settings"}
            </h1>
            <p className="mt-1 text-sm text-slate-500">
              {counts.visible} visible, {counts.hidden} hidden, {counts.total} total pages.
            </p>
          </div>
